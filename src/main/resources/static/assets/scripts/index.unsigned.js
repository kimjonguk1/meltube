...

{
    ...
    $loginForm.onsubmit = (e) => {
        ...
        url.pathname = '/user/' // localhost:8080/user/
    ...
    $menu.querySelector(':scope > .item > [rel="register"]').onclick = (e) => {
        ...
        $cover.show();
        $registerForm.reset(); // 창을 닫았다 켜도 필드에 내용을 안남아있게 하게
        $registerForm.show();
    ...
    $menu.querySelector(':scope > .item > [rel="recover"]').onclick = (e) => {
        ...
        $cover.show();
        $recoverForm.reset(); // 창을 닫았다 켜도 필드에 내용을 안남아있게 하게
        $recoverForm.show();
    }
}
...
$recoverForm.onsubmit = (e) => {
    ...
    if($recoverForm['mode'].value === 'password') {
        ...
        xhr.onreadystatechange = () => {
            ...
            }[response['result']] || ['요류', '서버가 알 수 없는 응답을 반환하였습니다. 잠시 후 다시 시도해주세요', ($dialog)=> Dialog.hide($dialog)];
            Dialog.show({
                title: title,
                content: content,
                buttons: [{
